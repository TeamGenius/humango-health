    ...
    func updateState(_ state: inout SleepSessionState, withSamples samples: [[String: Any]]) {
        for sample in samples {
            ...
            let durationSeconds: Double = {
                if let v = sample["durationSeconds"] as? Double { return v }
                if let v = sample["durationSeconds"] as? Int { return Double(v) }
                if let v = sample["durationMinutes"] as? Double { return v * 60.0 }
                if let v = sample["durationMinutes"] as? Int { return Double(v) * 60.0 }
                return 0
            }()
