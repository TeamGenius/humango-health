...
actor WorkoutRecordStore {
    ...
    private init() {
        if let data = UserDefaults.standard.data(forKey: "WorkoutRecordStore.records.v1") {
            do {
                let arr = try JSONDecoder().decode([Record].self, from: data)
                self.recordsById = Dictionary(uniqueKeysWithValues: arr.map { ($0.deviceActivityId, $0) })
            } catch {
